...
class GDrive():
    ...
    def create_user_token(self):
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                flow = InstalledAppFlow.from_client_secrets_file("secrets/u_credentials.json", SCOPES)
    ...
    def get_storage(self)-> pd.DataFrame:
        ...
        rem = (int(storage['storageQuota']['limit']) - int(storage['storageQuota']['usage']))/10**9
        ...
        s_data = {
            "Loc": ["Capacity", "Available", "Usage", "Drive Usage", "Trash Usage"],
            "Size in GB": [round(cap), round(rem), round(used), round(used_d), round(used_t)]
        }
